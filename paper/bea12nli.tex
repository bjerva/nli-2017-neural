...
Figure~\ref{fig:conf_mat} shows the confusion matrix of our best system's predictions (run 06).\todo[inline]{can we remove the bar on the right of the conf.plot? (the counts say it already all)}
...
Combining all of our systems without external data yields an F1 score of 83.23, which makes this the $n_{th}$\todo{update with real rank} system of the NLI Shared Task 2017.
...
\todo[inline]{Do we want to acknowledge anyone else?}
